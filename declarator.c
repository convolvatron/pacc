...
static void read_initializer_elem(parser p, scope env, vector inits, Type ty, boolean designated, index offset) {
    ...
    if (pget(ty, sym(kind)) == sym(array) || pget(ty, sym(kind)) == sym(struct)) {
        read_initializer_list(p, offset, env, inits, ty, designated);
...
value read_declarator_params(parser p, index offset, scope env, vector types)  {
    int length;
...
static Type read_declarator_tail(parser p, index offset, scope env, Type basety);
...
static Type read_declarator_array(parser p, index offset, scope env, Type basety) {
    ...
    Type t = read_declarator_tail(p, offset, env, basety);
